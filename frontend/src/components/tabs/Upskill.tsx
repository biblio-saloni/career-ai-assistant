...
export function Upskill({ data, extractedText }: Props) {
  ...
  const handleGenerateImprovedResume = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/api/resume/improve`, { method: "POST", body: formData });
      if (!response.ok) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: "0.72rem", fontWeight: 600, color: "#7c3aed", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "12px" }}>
            🔧 Skill Gaps
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {data.skill_gaps.map((gap, i) => (
              <div key={i} style={{
                ...
                <span style={{ fontSize: "0.83rem", color: "#e2e8f0", fontWeight: 500 }}>{gap.skill}</span>
                <span style={{
        ...
        <div style={{
          ...
          <div style={{ fontSize: "0.72rem", fontWeight: 600, color: "#dc2626", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "12px" }}>
            🚫 Missing ATS Keywords
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "7px" }}>
            {data.missing_keywords.map((kw, i) => (
              <span key={i} style={{
                fontSize: "0.78rem", padding: "5px 12px", borderRadius: "20px", fontWeight: 500,
                background: "rgba(248,113,113,0.08)", color: "#fca5a5",
                border: "1px solid rgba(248,113,113,0.2)",
              }}>+ {kw}</span>
            ))}
          </div>
          <p style={{ margin: "12px 0 0", fontSize: "0.76rem", color: "#475569", lineHeight: 1.5 }}>
            Add these naturally into your resume to pass ATS filters and reach more recruiters.
          </p>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "16px", flexWrap: "wrap", gap: "12px" }}>
          <div>
            <div style={{ fontSize: "0.72rem", fontWeight: 600, color: "#0e7490", textTransform: "uppercase", letterSpacing: "0.08em" }}>
              📝 Resume Improvements
            </div>
            <p style={{ margin: "4px 0 0", fontSize: "0.78rem", color: "#64748b" }}>Apply these to boost your ATS score and readability</p>
          </div>
          {!improvedText && (
            <button
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {data.improvements.map((item, i) => (
            <div key={i} style={{
              ...
              <span style={{
                minWidth: "22px", height: "22px", flexShrink: 0,
                background: "rgba(34,211,238,0.12)", color: "#22d3ee",
                borderRadius: "50%", display: "flex", alignItems: "center",
                justifyContent: "center", fontSize: "0.7rem", fontWeight: 700,
              }}>{i + 1}</span>
              <p style={{ margin: 0, fontSize: "0.83rem", color: "#cbd5e1", lineHeight: 1.6 }}>{item}</p>
            </div>
          ))}
        </div>
        ...
        {improving && (
          <div style={{ textAlign: "center", padding: "32px 0", color: "#64748b" }}>
            ...
            <p style={{ fontWeight: 600, margin: "0 0 4px", color: "#94a3b8" }}>AI is rewriting your resume...</p>
      ...
      {improvedText && (
        <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", flexWrap: "wrap", gap: "10px" }}>
            <div>
              <div style={{ fontWeight: 700, color: "#22c55e", fontSize: "0.95rem", display: "flex", alignItems: "center", gap: "8px" }}>
                ✅ Improved Resume Ready
              </div>
              <p style={{ margin: "4px 0 0", fontSize: "0.78rem", color: "#64748b" }}>
                Copy and paste this into Word, Google Docs, or your resume editor.
              </p>
            </div>
            <div style={{ display: "flex", gap: "8px" }}>
              <button
                ...
                onClick={handleCopy}
                style={{ background: copied ? "rgba(34,197,94,0.15)" : undefined, color: copied ? "#22c55e" : undefined }}
              >
                {copied ? "✅ Copied!" : "📋 Copy Text"}
              </button>
              <button
                className="improve-btn improve-btn--secondary"
                onClick={() => { setImprovedText(null); setError(null); }}
              >
                🔄 Regenerate
              </button>
            </div>
