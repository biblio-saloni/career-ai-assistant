...
public class JobScoringService {
        ...
        try {
            ...
            String response = webClient.get()
                    ...
                    .onRawStatus(status -> status >= 400,
                            clientResponse -> clientResponse.bodyToMono(String.class)
                                    .flatMap(body -> {
                                        logger.error("JSearch API error: status={}, body={}", clientResponse.getStatusCode(), body);
                                        return java.util.concurrent.Mono.error(
                                                new LlmServiceException("Job search failed: " + clientResponse.getStatusCode())
                                        );
                                    }))
            ...
            if (root.has("data") && root.get("data").isArray()) {
                for (JsonNode job : root.get("data")) {
                    Map<String, Object> jobMap = objectMapper.convertValue(job, new TypeReference<Map<String, Object>>() {});
                    jobs.add(jobMap);
                }
            }
        ...
        try {
            ...
            String response = webClient.post()
                    ...
                    .onRawStatus(status -> status >= 400,
                            clientResponse -> clientResponse.bodyToMono(String.class)
                                    .flatMap(body -> {
                                        logger.error("Groq API error: status={}", clientResponse.getStatusCode());
                                        return java.util.concurrent.Mono.error(
                                                new LlmServiceException("AI scoring failed. Please try again later.")
                                        );
                                    }))
                    .bodyToMono(String.class)
                    .block();
    ...
    private String buildJobSummaries(List<Map<String, Object>> rawJobs) {
        ...
        for (int i = 0; i < rawJobs.size(); i++) {
            ...
            String loc = String.join(", ",
                    ((String) job.getOrDefault("job_city", "")).isEmpty() ? List.of() : List.of((String) job.get("job_city")),
                    ((String) job.getOrDefault("job_state", "")).isEmpty() ? List.of() : List.of((String) job.get("job_state")),
                    ((String) job.getOrDefault("job_country", "")).isEmpty() ? List.of() : List.of((String) job.get("job_country"))
            ).replaceAll("^, |, $", "");
    ...
    private List<Map<String, Object>> parseGroqScoredJobs(String response, List<Map<String, Object>> rawJobs) throws Exception {
        JsonNode root = objectMapper.readTree(response);
